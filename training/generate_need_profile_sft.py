...
Merges existing golden/HR export with synthetic diversity rows.
"""
...
def _profile(
    *,
    ...
    if category_label:
        pref_list.append({"concept": category_label, "importance": 0.9})
    if category_hint:
    ...
    if clarify:
        ...
        profile["ambiguities"] = ambiguities or [
...
def iter_synthetic(target: int, *, seed: int = 42) -> Iterator[dict[str, Any]]:
    ...
    neg_n = int(target * 0.15)
    for i in range(neg_n):
        cat_label, hint, aliases = CATEGORIES[i % len(CATEGORIES)]
        alias = aliases[i % len(aliases)]
        neg_key, neg_phrases = NEGATIONS[i % len(NEGATIONS)]
        utterance = f"{alias} istiyorum ama {neg_phrases[i % len(neg_phrases)]} (#{i})"
        profile = _profile(
            utterance=utterance,
            category_label=cat_label,
            category_hint=hint,
            ambiguities=[
                {"code": "negative_constraint", "description": f"exclude:{neg_key}"}
            ],
            confidence=0.8,
        )
        yield emit(f"syn-neg-{i}", utterance, profile)
        produced += 1

    extras = [
        ("Şu iki laptopu karşılaştırmak istiyorum", "COMPARE_OPTIONS"),
    ...
    while produced < target:
        utterance_base, intent = extras[i % len(extras)]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=_ROOT / "training" / "exports" / "need_profile_sft.v2.jsonl",
    )
    ...
    if not args.no_seed_export:
        for row in list(iter_golden_sft_rows()) + list(
    ...
    budgets = sum(
        1 for r in kept if (r["need_profile"].get("budget") or {}).get("type") != "UNKNOWN"
    )
    print(
        json.dumps(
            {
    ...
    print(
        "NOTE: synthetic expansion — no QUALITY_READY claim; no merchant/bank fixture IDs.",
        file=sys.stderr,
    )
    return 0
