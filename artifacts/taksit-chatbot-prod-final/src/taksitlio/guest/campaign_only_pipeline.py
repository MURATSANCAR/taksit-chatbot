...
def _fallback_lexical(text: str) -> Optional[Any]:
    ...
    patterns = [
        ...
        (
            r"\b(cep\s*telefon|ak[ıi]ll[ıi]\s*telefon|\btelefon\b|\biphone\b)\b",
            "MOBILE_PHONE",
            "Cep Telefonu",
        ),
    ]
...
class CampaignOnlyGuestPipeline:
    ...
    async def start(self, *, locale: str = "tr-TR") -> dict[str, Any]:
    ...
    async def handle(
        ...
        *,
        ...
        try:
            ...
            return await self.start(locale=locale)
    ...
    async def _rank_campaigns(self, ctx: GuestContext) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # Fallback: all active (still campaigns only — never products)
            if not raw and hasattr(self._campaigns, "list_by_category_codes"):
                # try broad retail codes if white goods unseeded
                raw = list(
                    await self._campaigns.list_by_category_codes(
                        ["1", "MOBILE_PHONE", "WHITE_GOODS"], limit=50
                    )
                )
