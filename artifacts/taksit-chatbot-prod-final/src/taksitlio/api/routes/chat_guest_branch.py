...
async def run_guest_branch(payload: Any, container: Any) -> dict:
    ...
    if is_hello and (
        not sid or sid in ("new", "null", "") or revision in (None, 0)
    ):
        return await adapter.start_guest_session(locale="tr-TR")

    expected_revision = revision or 0
    if not sid or sid in ("new", "null", ""):
        ...
        # If first message was a need (not hello), continue on same session
