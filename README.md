...
* [`docs/adr/ADR-009-real-runtime-verification-and-provisional-acceptance.md`](docs/adr/ADR-009-real-runtime-verification-and-provisional-acceptance.md)
* [`docs/adr/ADR-012-answer-integrity-claim-grounding-and-recommendation-safety.md`](docs/adr/ADR-012-answer-integrity-claim-grounding-and-recommendation-safety.md)
...
**ADR-008 P1 / ADR-009 (runtime verification):** Matcher quality locked. Real Redis + pgvector integration suites (CI `skipped=0`), `RemoteFastExtractor`, `StrictOpenAICompatibleEmbedder` (no lexical fallback), dependency probes, and provisional gate. Live FAST/embedding endpoints are env-configured only — absence → `BLOCKED_DEPENDENCY`, never silent stub success. Compose: `docker compose -f docker/docker-compose.runtime.yml --profile runtime-verification up -d`. **Canlı sunucu runbook:** [`docs/runbooks/ADR-009-live-runtime-verification.md`](docs/runbooks/ADR-009-live-runtime-verification.md). Reports: `evaluation/reports/adr008-p1-*.json`. Campaign remains `CLOSED` until `PROVISIONAL_ACCEPT`.

**ADR-012 (answer integrity):** Claim grounding, payment reconciliation, circuit breakers, sponsored isolation. Gates PASS in unit/acceptance. Live migrate + smoke: [`docs/runbooks/ADR-012-answer-integrity-ops.md`](docs/runbooks/ADR-012-answer-integrity-ops.md).
